...
class ExtensionService(SSE.ConnectorServicer):
    ...
    def _prophet(request, context):
                ...
                for item in adjJson:
                    ...
                    if dt == forecast.at[index, 'ds']:
                        ...
                        if "%" in adjustmentString:
                            ...
                            forecast.at[index, forecastReturnType] = float(forecast.at[index,forecastReturnType]) * adjustmentPercent
                        else:    
                            forecast.at[index, forecastReturnType] = float(forecast.at[index,forecastReturnType]) + float(adjustmentString)
        except:
        ...
        forecast = pd.merge(index, forecast, how='outer',on='ds', left_index=False, right_index=False, sort=False)
